...
class DBpediaSpotlight extends BaseAPI {
	...
	public function init_nlp($text) {
		...
		$this->api_args = array(
			...
			'confidence' => !empty($GLOBALS['api_config']['dbpedia']['confidence']) ? $GLOBALS['api_config']['dbpedia']['confidence'] : 0.4,
			'support' => !empty($GLOBALS['api_config']['dbpedia']['support']) ? $GLOBALS['api_config']['dbpedia']['support'] : 20
		);
	}
